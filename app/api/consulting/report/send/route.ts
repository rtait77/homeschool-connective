...
export async function POST(req: NextRequest) {
  ...
  function buildItemHtml(item: typeof typedItems[0], idx: number) {
    ...
    const tagHtml = allPeople.length > 1
      ? allPeople.map((p: string) => `<span style="font-size:11px;font-weight:700;padding:1px 9px;border-radius:999px;background:${p === 'Parent' ? '#f9d0e8' : '#e8d9ff'};color:${p === 'Parent' ? '#9d174d' : '#6d28d9'};margin-right:4px;">${p === 'Parent' ? 'Parent/Guardian' : p}</span>`).join('')
      : ''
    return `
